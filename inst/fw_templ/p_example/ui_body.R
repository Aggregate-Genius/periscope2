...
style_guide_box <- box(
    ...
    tags$li("There are many theme variables are available for customization. Review ",
            ...
            " for more info"),
    tags$li("Fortunately, periscope2 provids a user friendly method to update fresh theme variable via configurations,
            rather than by updating them programatically, by only updating 'www/periscope_style.yaml' values and relunch app"),
    tags$li("'periscope_style.yaml' contains the most important variables with documentation explaining what each variable to and possible value for it",
            tags$ul(tags$li("There is another in progress tool for managing all fresh theme variables and will be presented in next periscope2 versions."))),
    tags$li("Another advanced method to update application themes or any controls CSS is by updating 'www/css/custom.css' file"),
    tags$li("The below widget is explaining different theme variables and how they can affect the application"),
    hr(),
    tags$dt("Status Colors"),
    tags$li("Sets the status colors that affects the color of the header, valueBox, infoBox and box"),
    tags$li("Valid values are names of the color or hex-decimal value of the color (i.e,: 'blue', '#086A87')"),
    tags$li("Blank/empty values will use the default values"),
    br(),
    fluidRow(column(width = 6,
    ...
    tags$li("Blank/empty values will use default values"),
    br(),
    fluidRow(column(width = 6,
...
add_ui_body(tabItems(application_setup, periscope_modules, user_notifications, style_guide))
