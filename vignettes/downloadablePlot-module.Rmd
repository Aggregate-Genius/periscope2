...
The **downloadablePlot** Shiny Module is a part of the *periscope2* package and 
consists of the following functions:

* **downloadablePlotUI** - the UI function to place the plot in the application
* **downloadablePlot** - the server function to be called inside server_local.R.
...
The downloadtypes and download_hovertext arguments are passed to the downloadFileButton and set the file types the button will allow the user to
...
**Note**: When you create an object using any of these functions shiny will send 
...
* the unique object ID that was provided to downloadablePlotUI when creating the UI object
...
* a **named list** of functions or reactive expressions that provide the images or data to the 
downloadFileButton (see below).  
  * It is important that the types of files to be downloaded are matched to the correct named function in the list.  
  * The function/reactive names are unquoted - they will be called at the time the user initiates a 
...
* If a function is provided it must be parameterless (require NO parameters).  No parameters will be provided when a function is called to retrieve the plot or data.  Reactive expressions cannot take parameters by definition.
...
* For the visible plot the return value must be from the ggplot2, grob or lattice classes (or a superclass therein)
