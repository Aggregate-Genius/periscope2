...
react_table_box <- box(
    ...
    fluidRow(column(width = 6,
                    tags$dl(tags$dt("Features"),
                            tags$ul(tags$li("React table downloader module displays tabular data in rich formatted tables using the", tags$b("`reactable`"), "package"),
                                    tags$li("downloadableReactTable returns a named reactive list that can be used in the application:",
                                            tags$ul(tags$li(tags$b("selected_rows"), ": data.frame of current selected rows"),
                                                    tags$li(tags$b("table_state"), ": a list of the current table state"))),
                                    tags$li("Table data can be downloaded in different formats such as: ", tags$b("'csv'"),
                                            ", ", tags$b("'tsv'"), ", ", tags$b("'txt'"), "and/or ", tags$b("'xlsx'")),
                                    tags$li("User can customize downloadableReactTable modules using reactable package options."),
                                    tags$li("For more information about table options please visit the",
                                            tags$a("Reactable documentation", target = "_blank", href = "https://glin.github.io/reactable/"),
                                            "site")
                            ))),
             column(width = 6,
                    tags$dl(tags$dt("Setup"),
                            tags$li("Module should be configured in both UI and Server code"),
                            tags$li("In the 'body_ui.R' add the following lines:",
                                    blockQuote("downloadableReactTableUI('exampleReactTable',
                                                                         'Download react table data'))", color = "info")),
                            tags$li("In the 'server_local.R' add the following lines (make sure the two use the same ID):",
                                    blockQuote("downloadableReactTable('exampleReactTable',
                                                                       ss_userAction.Log,
                                                                       'exampletable',
                                                                        list(csv = load_data3, tsv = load_data3))", color = "info")),
                            tags$li("Review ", tags$b("?downloadableReactTableUI"), " and ", tags$b("?downloadableReactTable"), " for more information"),
                            tags$li("Review below table for detailed example code")))),
    fluidRow(column(width = 12,
                    downloadableReactTableUI(id            = "exampleReactTable",
                                             downloadtypes = list("csv", "tsv"),
                                             hovertext     = "Download react table data")))
...
alerts_box <- box(
    ...
    blockQuote(tags$pre("S:createPSAlert(id = 'alert place',
...
add_ui_body(tabItems(application_setup, periscope_modules, user_notifications))
