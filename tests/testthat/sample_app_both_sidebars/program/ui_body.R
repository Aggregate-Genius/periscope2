...
react_table_box <- box(
    ...
    fluidRow(column(width = 6,
                    tags$dl(tags$dt("Features"),
                            tags$ul(tags$li("React table downloader module display tabular data in rich formatted tables using", tags$b("`reactable`"), "package"),
                                    tags$li("downloadabkeReactTable returns two names reactive list that can be used in application different areas",
                                            tags$ul(tags$li(tags$b("selected_rows"), ": data.frame of current selected rows"),
                                                    tags$li(tags$b("table_state"), ": a list of current rendered table state values."))),
